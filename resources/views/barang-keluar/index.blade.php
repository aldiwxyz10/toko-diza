<x-app-layout>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
        ...
        <div class="px-6 py-4 border-b border-slate-100 bg-slate-50/50">
            <form action="{{ route('barang-keluar.index') }}" method="GET" class="flex flex-col md:flex-row gap-3 items-center">
                <!-- Search Input -->
                <div class="flex items-center border pl-3 gap-2 bg-white border-slate-200 focus-within:border-blue-500 focus-within:ring-2 focus-within:ring-blue-100 h-[42px] rounded-lg overflow-hidden w-full md:w-96 transition-all">
                    ...
                    @if(request('search'))
                        <a href="{{ route('barang-keluar.index') }}" class="pr-3 text-slate-400 hover:text-slate-600 transition-colors" title="Bersihkan Pencarian">
